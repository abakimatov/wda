...
import { TKnbk } from '../../../types/reducers/typesDayliReportReducer';
import { SET_KNBK } from '../../../types/actions/actionsDayliReportReducer';
import { Methods } from '../../Methods/methods';
//PLANS
export const getDRKnbk = (flag?: string, id?: number) : ThunkType => async(dispatch, getState) => {
    dispatch({ type: LOADING_DATA });
    const docId = getState().data.urlParams.id;
    try {
        const res = await instance(check_current()).post(``, { method: Methods.getDRKnbk, flag, id, docId });
        const data = res.data;
        dispatch({ type: SET_KNBK, flag, id, payload: convertNull(data.list) });
...
export const iudDRKnbk = (flag?: string, body?: TKnbk, functionResultPromise?: (result?: boolean, id?: number) => void) : ThunkType => async(dispatch, getState) => {
    dispatch({ type: LOADING_DATA });
    const docId = getState().data.urlParams.id;
    try {
        const res = await instance(check_current()).post(``, { method: Methods.iudDRKnbk, flag, docId, ...body });
        const data = res.data;
        dispatch(getDRKnbk(null, null));
